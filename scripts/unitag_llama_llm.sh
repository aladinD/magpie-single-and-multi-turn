...
tag_mission=${2:-"all"}
device=${3:-"0"}
model_path=${4:-"meta-llama/Llama-3.3-70B-Instruct"}
reward_device=${5:-"0"}
...
if [ $tag_mission == "difficulty" ] || [ $tag_mission == "all" ]; then
    ...
    CUDA_VISIBLE_DEVICES=$device python ../exp/my_unitag.py \
        --device $device \
...
if [ $tag_mission == "llm_classification" ] || [ $tag_mission == "all" ]; then
    echo "[magpie.sh] Start Generating LLM-Task Tags..."
    CUDA_VISIBLE_DEVICES=$device python ../exp/my_unitag.py \
        --device $device \
        --model_path $model_path \
        --input_file $input_file \
        --tag_mission "llm_classification" \
        --tensor_parallel $tensor_parallel \
        --gpu_memory_utilization $gpu_memory_utilization \
        --batch_size $batch_size \

    echo "[magpie.sh] Finish Generating LLM Task Tags!"
    ...
    input_file_name_no_ext="${input_file_name%.*}"
    input_file_ext="${input_file_name##*.}"
    task_tag_file="${input_file_dir}/${input_file_name_no_ext}_llm_category.${input_file_ext}"
    input_file=$task_tag_file
    echo "[magpie.sh] LLM Task Tagged File: $input_file"
fi
...
if [ $tag_mission == "llm_reward" ] || [ $tag_mission == "all" ]; then
    echo "[magpie.sh] Start Generating LLM-Reward Tags…"
    CUDA_VISIBLE_DEVICES=$device python ../exp/my_unitag.py \
        --device $device \
        --model_path $model_path \
        --input_file $input_file \
        --tag_mission "llm_reward" \
        --tensor_parallel $tensor_parallel \
        --gpu_memory_utilization $gpu_memory_utilization \
        --batch_size $batch_size

    echo "[magpie.sh] Finish Generating LLM-Reward Tags!"
    ...
    input_file_name_no_ext="${input_file_name%.*}"
    input_file_ext="${input_file_name##*.}"
    llm_reward_tag_file="${input_file_dir}/${input_file_name_no_ext}_llm_reward.${input_file_ext}"
    input_file=$llm_reward_tag_file
    echo "[magpie.sh] LLM-Reward Tagged File: $input_file"
fi
